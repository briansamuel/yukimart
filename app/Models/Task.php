...
class Task extends Model
{
    ...
    protected function firstLetterName(): Attribute
    {
        return new Attribute(
            get: fn ($value,  $attributes) => mb_substr($attributes['task_name'], 0, 1),
        );
    }
    ...
    protected function projectStatus(): Attribute
    {
        return Attribute::make(
            get: function($value,  $attributes) {
                $dueDate = Carbon::parse($attributes['task_due_date']);
                $nowDate = Carbon::now();
    ...
    protected function badgeStatus(): Attribute
    {
        return Attribute::make(
            get: function($value,  $attributes) {
                ...
                $nowDate = Carbon::now();
                $result = $nowDate->gt($dueDate);
                $status = $result && $attributes['task_status'] == 'in_progress' ? 'over_due' : $attributes['task_status'];
     ...
     * @return \Illuminate\Database\Eloquent\Casts\Attribute
     */
    protected function taskDueDate(): Attribute
    {
        return Attribute::make(
    ...
    {
        return $this->hasMany(User::class, 'id',  'created_by_user');
    }
}
