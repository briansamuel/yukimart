...
class UpdateCustomerRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($this->has('phone') && $this->phone && !preg_match('/^(0|\+84)[0-9]{9,10}$/', $this->phone)) {
                $validator->errors()->add('phone', 'Phone number format is invalid');
            }

            // Validate tax code format (Vietnamese tax code) if provided
            if ($this->has('tax_code') && $this->tax_code && !preg_match('/^[0-9]{10,13}$/', $this->tax_code)) {
                $validator->errors()->add('tax_code', 'Tax code must be 10-13 digits');
            }

            // If both phone and email are being set to null, prevent it
            $customer = $this->route('customer');
            $newPhone = $this->has('phone') ? $this->phone : $customer->phone;
            $newEmail = $this->has('email') ? $this->email : $customer->email;
